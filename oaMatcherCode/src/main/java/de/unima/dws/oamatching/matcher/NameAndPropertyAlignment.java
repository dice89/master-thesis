...
public class NameAndPropertyAlignment extends BaseMatcher {
	final static Logger logger = LoggerFactory
			.getLogger(NameAndPropertyAlignment.class);
	...
	@Override
	public List<Cell> align_match(double threshold) {
		List<Cell> aligned_cells = new ArrayList<Cell>();
		try {
			align(null, new Properties());
		} catch (AlignmentException e) {
			e.printStackTrace();
		}
		Enumeration<Cell> enumerator = getElements();

		while (enumerator.hasMoreElements()) {
			Cell cell = (Cell) enumerator.nextElement();
			aligned_cells.add(cell);
		}
		postPrune(threshold);
	 ...
	 * (non-Javadoc)
	 * 
	 * @see de.unima.dws.omatching.matcher.BaseMatcher#prepare(java.net.URI,
	 * java.net.URI)
	 */
	@Override
	public void prepare(URI onto1, URI onto2) {
		cleanUp();
		setType("**");

		OWLAPI3OntologyFactory ontofactory = new OWLAPI3OntologyFactory();

		OWLAPI3Ontology loaded1 = null;
		OWLAPI3Ontology loaded2 = null;
		try {
			loaded1 = (OWLAPI3Ontology) ontofactory.loadOntology(onto1, false);
			loaded2 = (OWLAPI3Ontology) ontofactory.loadOntology(onto2, false);
			init(loaded1, loaded2);
		} catch (Exception e) {
			e.printStackTrace();
		}
	 ...
	 */
	public void init(Object o1, Object o2, Object ontologies)
	...
	public void align(Alignment alignment, Properties params)
			...
			for (i = 0; i < nbprop1; i++) {
				...
				for (j = 0; j < nbprop2; j++) {
					...
					if (st1 != null || st2 != null) {
						propmatrix[i][j] = pia1
								* StringDistances.subStringDistance(st1, st2);
					} else {
						propmatrix[i][j] = 1.;
					}
				}
			...
			for (i = 0; i < nbclass1; i++) {
				...
				for (j = 0; j < nbclass2; j++) {
					classmatrix[i][j] = pic1
							* StringDistances.subStringDistance(honto1
									.getEntityName(cl1).toLowerCase(), honto2
									.getEntityName(classlist2.get(j))
									.toLowerCase());
				}
			}
		...
		while (factor > epsillon) {
			...
			for (i = 0; i < nbprop1; i++) {
				...
				if (found && max < 0.5) {
					addAlignCell(proplist1.get(i), proplist2.get(best), "=",
							1. - max);
				}
			}
			logger.debug("Computing class distances");
			// Compute classes distances
			// -- for all of its attribute, find the best match if possible...
			// easy
			...
			try {
				for (i = 0; i < nbclass1; i++) {
					Set<? extends Object> properties1 = honto1.getProperties(
							classlist1.get(i), OntologyFactory.ANY,
							OntologyFactory.ANY, OntologyFactory.ANY);
					...
					if (nba1 > 0) { // if not, keep old values...
						...
						for (j = 0; j < nbclass2; j++) {
							...
							double moy_align_loc = alignLocal(properties1,
									properties2);
		...
		for (i = 0; i < nbclass1; i++) {
			...
			if (found && max < 0.5) {
				addAlignCell(classlist1.get(i), classlist2.get(best), "=",
						1. - max);
			}
		}
	}
	 ...
	 */
	private double alignLocal(Set<? extends Object> prop1,
			Set<? extends Object> prop2) {
		...
		for (Object property1 : prop1) {
			...
			for (Object property2 : prop2) {
				...
				try {
					...
					propmatrix[i][j] = 1.0 - StringDistances.subStringDistance(
							s1, s2);
				} catch (OntowrapException aex) {
					propmatrix[i][j] = 0.0;
				}

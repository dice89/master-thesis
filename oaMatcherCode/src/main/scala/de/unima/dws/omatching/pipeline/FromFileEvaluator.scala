...
object FromFileEvaluator {
  ...
  def main(arg: Array[String]): Unit = {
    val file: File = new File("result.csv")
  	...
  	val alignment = Pipeline.combineMatchingsMatrix( Pipeline.readCSV(file),0.95)  
    ...
    val res = Pipeline.validate("ontos/2014/conference/reference-alignment/cmt-confOf.rdf")(alignment)

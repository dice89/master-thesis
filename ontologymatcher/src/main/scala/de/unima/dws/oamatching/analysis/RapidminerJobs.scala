...
object RapidminerJobs {
  ...
  def rapidminerOutlier(writeFunction: FeatureVector => File)(readFunction: File => Map[MatchRelation, Double])(matchings: FeatureVector): Map[MatchRelation, Double] = {
    ...
    //TODO set parameters in selection attributes so that it's dynamic

    process.getOperator("Input").setParameter("csv_file", input_csv.getAbsolutePath)
    val test:java.util.List[Array[String]] = new java.util.ArrayList[Array[String]]()
    test.add(Array("a","b","c"))
    test.add(Array("a","b","c"))

    process.getOperator("Input").setListParameter("data_set_meta_data_information",test)
